...
This repository currently documents the DevRel Agent design in `DEVREL_AGENT_IMPLEMENTATION.md`. If you implement code, follow the planned layout in that document: a top-level `prism-devrel/` project with `src/devrel/` and subpackages like `core/`, `github/`, `llm/`, `vector/`, and `agents/`. Keep the plan updated whenever you add, rename, or remove modules so the diagram and examples stay accurate.
...
If you add scripts, list them here and in `DEVREL_AGENT_IMPLEMENTATION.md` with a one-line purpose.
...
This directory is not a Git repo, so no commit history is available. Use clear, imperative messages with a short scope (for example, `docs: update architecture diagram` or `feat: add GitHub client scaffold`). PRs should include a concise description, any linked issue, and a note about updates required in `DEVREL_AGENT_IMPLEMENTATION.md`.
