...
### 입력
- `ContextBundle`
  - 이벤트 목록(시간, actor, type, payload) — 권장 스키마: `phase3/context-bundle.schema.json`
  - 대화/툴콜/코드변경/에러로그/결과 요약 등
  - (선택) 사건 메타: 프로젝트, 저장소, 브랜치, 태그, 성공/실패 여부

### 출력
- DB에 저장되는 레코드
  - 사건 요약/핵심 쟁점
  - 역할별 교훈(텍스트 + 구조화 메타 + 임베딩)
  - 판결 기록(왜 저장/보류했는지)
  - (선택) 역할별 프롬프트 업데이트 제안 및 적용 이력
...
5. **판사(Judge)**: 최종 판결
   ...
   - (b) 역할별 프롬프트 업데이트가 필요하다면 “제안” 생성(적용은 휴먼 승인 이후)
6. **저장/색인**
...
2. Court 4에이전트 1회 실행 → Judge가 `selected_lessons` 산출
3. 교훈 임베딩 생성 → `lessons` 저장
4. `search_lessons(role, query)` API(또는 함수)로 Top-K 조회
5. 프롬프트 업데이트는 `prompt_updates`에 “proposed”로만 저장(자동 적용은 차기)
