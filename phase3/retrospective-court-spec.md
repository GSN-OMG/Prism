...
병렬 개발을 위한 분할 스펙: `specs/00-index.md`
...
- **민감정보 처리**: 정의서를 별도로 둔다. 초기에는 디폴트 정책(JSON)을 제공하고 사용자가 업데이트 가능하도록 한다. (`sensitive-info-spec.md`)
...
### 입력
- `ContextBundle` (권장 스키마: `context-bundle.schema.json`)
...
1. **케이스 생성/정규화**
   ...
   - 민감정보(PII/API 키) 마스킹 정책 적용(디폴트: `redaction-policy.default.json`)
